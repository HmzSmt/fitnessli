...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($date_fin < $date_debut) {
        echo '<script>alert("La date de fin ne doit pas être antérieure à la date de début.");</script>';
        echo '<script>window.location.href = "modifier.php";</script>';
        exit;
    }
    
    if ($date_debut < date('Y-m-d', strtotime('+1 day'))) {
        echo '<script>alert("La date de début ne peut pas être dans le passé ou aujourd\'hui.");</script>';
        echo '<script>window.location.href = "modifier.php";</script>';
        exit;
    }

    // Mise à jour de l'événement dans la base de données
    $sql = "UPDATE evenements SET titre = ?, description = ?, date_debut = ?, heure_debut = ?, date_fin = ?, heure_fin = ? WHERE id = ?";
    $stmt = $pdo->prepare($sql);
